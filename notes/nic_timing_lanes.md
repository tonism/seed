...
2026-06-02 - LONG-RENDER 0D RESOLVED (supersedes the 05-30 "0D/12 = (re)handshake Certificate-receive timeout" note). A wire capture of a failing ne1k long run showed the answer delivered + rendered in FULL with a clean monotonic TCP stream (no loss/retransmit); the 0D was a COMPLETION-MARKER MISS -> the receive loop keeps waiting for more -> spurious MID-EXCHANGE reconnect -> the reconnect's full TLS handshake (~14.8s on 8088: ServerHello, CKE +7s ECDHE, Finished +7.7s key-block) races Cloudflare's ~15s window -> 0D. So the 05-30 "handshake Certificate-receive timeout" was the SYMPTOM (the reconnect handshake), not the ROOT (the completion miss that forced the reconnect). FIX (commit 535de35): extend agent_api_exchange's .text_seen_fallback to ALL NICs (was family_3c501-only) - on a receive/ack failure after text rendered, accept the answer + invalidate the session instead of reconnecting mid-exchange. 3c501 already had it (hence it was 4/4 long); generalizing removes the spurious reconnect. The 05-30 proposed hardening (raise tcp_payload_wait_count / auto-retry the handshake) is NOT needed for the long-render case. VALIDATION: 7-card matrix long 0D 3/28 -> 0/21 (overnight 0/17 + caffeinated re-confirm 4/4 on flake-prone ne1k/novell-ne1k/wd8003eb); short+idle solid; cold connects 28/28. TESTING-ARTIFACT: the intermittent "no response"/hang seen 06-01/02 is a HARNESS screencapture observation fault (screenshot_86box_window / run() lack a timeout -> a blocked capture freezes detection -> watchdog -> false no-response), NOT the ROM (no screencapture in the product path; committed-stable flaked identically; VM renders fine by eye). See docs/builds.md (Build 8 checkpoint).
