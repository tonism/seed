...
bounded DHCPACK wait to mark the lease accepted. After DHCPACK, it sends ARP
for the DHCP-provided DNS server and records the resolved destination MAC. It
then sends a minimal DNS query and waits for a matching response. Outbound
transport reachability is next.
...
vm-net-ne1k          NE1000-compatible; expected: adapter prompt, MAC read, RX read check, DHCPDISCOVER/OFFER, DHCPREQUEST/ACK, DNS ARP/query, then seed build 5
vm-net-ne2k8         8-bit NE2000-compatible; expected: adapter prompt, MAC read, RX read check, DHCPDISCOVER/OFFER, DHCPREQUEST/ACK, DNS ARP/query, then seed build 5
vm-net-novell-ne1k   Novell NE1000; expected: adapter prompt, MAC read, RX read check, DHCPDISCOVER/OFFER, DHCPREQUEST/ACK, DNS ARP/query, then seed build 5
