...
DHCPDISCOVER transmit, and a bounded packet-filtered DHCPOFFER wait. The NE
receive path records separate DMA, ring-header, and byte-count failures so DHCP
wait behavior can be diagnosed without changing user-facing text. Nonmatching
packets are discarded while the wait remains bounded. When status is 7, the IP,
router, and DNS fields contain byte-order IPv4 values copied from the offer.
That is not yet an accepted lease; DHCPREQUEST/ACK handling remains in build 5
scope. If no offer is observed during the bounded wait, the boot can still
continue with status 6 and network error 4.
